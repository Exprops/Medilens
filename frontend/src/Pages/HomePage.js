...
import WebcamCapture from '../components/WebcamCapture'; // webcam component import
...
function HomePage() {
    ...
    const handleUpload = () => {
        const hasImage = selectedImage !== null || webcamImage !== null;
        const hasText = userPrompt.trim() !== '';

        if (!hasText && !hasImage) {
            setError("Please enter a message or select/capture an image to analyze.");
            toast({
                title: "Input Required",
                description: "Please provide text or an image.",
                variant: "warning",
        ...
        if (hasImage) {
            ...
            uploadMutation.mutate({ imageFile: imageToSend, textPrompt: userPrompt });
        } else {
            chatMutation.mutate(userPrompt);
        }
    };
    ...
    const initLeafletMapAndFindHospitals = async () => {
        ...
        try {
            ...
            const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/leaflet-hospitals`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ latitude: location.lat, longitude: location.lng, radius_km: 5 })
            });
        ...
        <div className="app-container">
            <div className="main-content-card">
                ...
                <div className="webcam-section">
                    <label>Or capture image from Webcam (Optional):</label>
                    <WebcamCapture
                        onCapture={(file) => {
                            setWebcamImage(file);
                            if (file) {
                ...
                {/* --- Hospital map & leaflet code as is - NO changes --- */}
                <div className="map-section">
                    <h2 className="map-title">
                        <MapPin className="map-pin-icon" /> Find Nearby Hospitals
                    </h2>
                        ...
                        <div className="hospital-list-container">
                            ...
                            <ul className="hospital-list">
                                {hospitalsList.map((hospital, index) => (
                                    <li key={hospital.osm_id || index} className="hospital-list-item">
                                        <strong>{hospital.name}</strong><br />
                                        {[hospital.address?.road, hospital.address?.suburb || hospital.address?.city, hospital.address?.state, hospital.address?.postcode]
                                            .filter(Boolean).join(', ')}
                                        <br />
